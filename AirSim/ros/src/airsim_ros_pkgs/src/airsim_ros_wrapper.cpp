...
void AirsimROSWrapper::initialize_airsim()
{
    ...
    try {
        ...
        get_object_pose_srvr_ = nh_private_.advertiseService<airsim_ros_pkgs::GetObjectPose::Request, airsim_ros_pkgs::GetObjectPose::Response>("get_object_pose",
                                                                                               boost::bind(&AirsimROSWrapper::get_object_pose_srv_cb, this, _1, _2));

        set_object_pose_srvr_ = nh_private_.advertiseService<airsim_ros_pkgs::SetObjectPose::Request, airsim_ros_pkgs::SetObjectPose::Response>("set_object_pose",
                                                                                               boost::bind(&AirsimROSWrapper::set_object_pose_srv_cb, this, _1, _2));
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            drone->move_on_path_srvr = nh_private_.advertiseService<airsim_ros_pkgs::MoveOnPath::Request, airsim_ros_pkgs::MoveOnPath::Response>(curr_vehicle_name + "/move_on_path",
        ...
        camera_publisher.publisher = nh_private_.advertise<airsim_ros_pkgs::IntrinsicsCamera>(curr_vehicle_name + "/" + ""+ "/intrinsics", 10);
        ...
        camera_subscriber.subscriber = nh_private_.subscribe<airsim_ros_pkgs::IntrinsicsCamera>(curr_vehicle_name + "/" + ""
        		+ "/intrinsics", 1, boost::bind(&AirsimROSWrapper::update_intrinsics_cb, this, _1, curr_vehicle_name, ""));
        vehicle_ros->intrinsics_sub_vec.push_back(camera_subscriber);

        vehicle_ros->enable_manual_focus_srv_vec.push_back(nh_private_.advertiseService<airsim_ros_pkgs::EnableManualFocus::Request, airsim_ros_pkgs::EnableManualFocus::Response>(curr_vehicle_name + "/" + "" + "/" + "enable_manual_focus",
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
        	...
        	camera_publisher.publisher = nh_private_.advertise<airsim_ros_pkgs::IntrinsicsCamera>(curr_vehicle_name + "/" + curr_camera_name+ "/intrinsics", 10);
        	vehicle_ros->intrinsics_pub_vec.push_back(camera_publisher);

        	IntrinsicsSubscriber camera_subscriber;
        	camera_subscriber.camera_name = "";
        	camera_subscriber.subscriber = nh_private_.subscribe<airsim_ros_pkgs::IntrinsicsCamera>(curr_vehicle_name + "/" + curr_camera_name
        	                		+ "/intrinsics", 1, boost::bind(&AirsimROSWrapper::update_intrinsics_cb, this, _1, curr_vehicle_name, curr_camera_name));
            vehicle_ros->intrinsics_sub_vec.push_back(camera_subscriber);


        	 vehicle_ros->enable_manual_focus_srv_vec.push_back(nh_private_.advertiseService<airsim_ros_pkgs::EnableManualFocus::Request, airsim_ros_pkgs::EnableManualFocus::Response>(curr_vehicle_name + "/" + curr_camera_name + "/" + "enable_manual_focus",
        	                                                                                                                                             boost::bind(&AirsimROSWrapper::enable_manual_focus_srv_cb, this, _1, _2, curr_vehicle_name, curr_camera_name)));
...
void AirsimROSWrapper::update_intrinsics_cb(const airsim_ros_pkgs::IntrinsicsCamera::ConstPtr& intrinsics_cmd_msg, const std::string& vehicle_name, const std::string& camera_name)
{
	string info = camera_name + "  " + vehicle_name;
	ROS_INFO_STREAM("Msg"<<info);
...
bool AirsimROSWrapper::enable_manual_focus_srv_cb(airsim_ros_pkgs::EnableManualFocus::Request& request, airsim_ros_pkgs::EnableManualFocus::Response& response, const std::string& vehicle_name, const std::string& camera_name)
{
    static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->simEnableManualFocus(request.enable, camera_name, vehicle_name);

    return true;
}

bool AirsimROSWrapper::get_object_pose_srv_cb(airsim_ros_pkgs::GetObjectPose::Request& request, airsim_ros_pkgs::GetObjectPose::Response& response)
{
   Pose p =  static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->simGetObjectPose(request.object_name);

   response.object_pose.position.x = p.position.x();
   response.object_pose.position.y = p.position.y();
   response.object_pose.position.z = p.position.z();
   response.object_pose.orientation.x = p.orientation.x();
   response.object_pose.orientation.y = p.orientation.y();
   response.object_pose.orientation.z = p.orientation.z();
   response.object_pose.orientation.w = p.orientation.w();
...
bool AirsimROSWrapper::set_object_pose_srv_cb(airsim_ros_pkgs::SetObjectPose::Request& request, airsim_ros_pkgs::SetObjectPose::Response& response)
{
   Pose p =  static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->simGetObjectPose(request.object_name);

   p.position.x() = request.object_pose.position.x;
   p.position.y() = request.object_pose.position.y;
   p.position.z() = request.object_pose.position.z;

   p.orientation.x() = request.object_pose.orientation.x;
   p.orientation.y() = request.object_pose.orientation.y;
   p.orientation.z() = request.object_pose.orientation.z;
   p.orientation.w() = request.object_pose.orientation.w;

   static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->simSetObjectPose(request.object_name, p);

   return true;
}

bool AirsimROSWrapper::move_on_path_srv_cb(airsim_ros_pkgs::MoveOnPath::Request& request, airsim_ros_pkgs::MoveOnPath::Response& response, const std::string& vehicle_name)
{
    ...
    static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->moveOnPathAsync(path, request.vel, request.timeout,
    		DrivetrainType::MaxDegreeOfFreedom, YawMode(false, request.rads_yaw),-1,1, vehicle_name);
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_car_state(const msr::airlib::CarApiBase::CarState& car_state) const
{
    ...
    if (isENU_) {
        std::swap(odom_msg.pose.pose.position.x, odom_msg.pose.pose.position.y); // @suppress("Invalid arguments")
        odom_msg.pose.pose.position.z = -odom_msg.pose.pose.position.z;
        std::swap(odom_msg.pose.pose.orientation.x, odom_msg.pose.pose.orientation.y);// @suppress("Invalid arguments")
        odom_msg.pose.pose.orientation.z = -odom_msg.pose.pose.orientation.z;
        std::swap(odom_msg.twist.twist.linear.x, odom_msg.twist.twist.linear.y);// @suppress("Invalid arguments")
        odom_msg.twist.twist.linear.z = -odom_msg.twist.twist.linear.z;
        std::swap(odom_msg.twist.twist.angular.x, odom_msg.twist.twist.angular.y);// @suppress("Invalid arguments")
...
void AirsimROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        for (auto& intrinsics_publisher : vehicle_ros->intrinsics_pub_vec) {
        	airsim_ros_pkgs::IntrinsicsCamera intrinsics_msg;
        	intrinsics_msg.aperture = airsim_client_->simGetFocusAperture(intrinsics_publisher.camera_name, vehicle_ros->vehicle_name, false);
        	intrinsics_msg.focal_length = airsim_client_->simGetFocalLength(intrinsics_publisher.camera_name, vehicle_ros->vehicle_name, false);
        	intrinsics_msg.focus_distance = airsim_client_->simGetFocusDistance(intrinsics_publisher.camera_name, vehicle_ros->vehicle_name, false);
        	intrinsics_publisher.publisher.publish(intrinsics_msg);
        }
...
void AirsimROSWrapper::append_static_vehicle_tf(VehicleROS* vehicle_ros, const VehicleSetting& vehicle_setting)
{
    ...
    if (isENU_) {
        std::swap(vehicle_tf_msg.transform.translation.x, vehicle_tf_msg.transform.translation.y);// @suppress("Invalid arguments")
        std::swap(vehicle_tf_msg.transform.rotation.x, vehicle_tf_msg.transform.rotation.y);// @suppress("Invalid arguments")
...
void AirsimROSWrapper::append_static_lidar_tf(VehicleROS* vehicle_ros, const std::string& lidar_name, const msr::airlib::LidarSimpleParams& lidar_setting)
{
    ...
    if (isENU_) {
        std::swap(lidar_tf_msg.transform.translation.x, lidar_tf_msg.transform.translation.y);// @suppress("Invalid arguments")
        std::swap(lidar_tf_msg.transform.rotation.x, lidar_tf_msg.transform.rotation.y);// @suppress("Invalid arguments")
...
void AirsimROSWrapper::append_static_camera_tf(VehicleROS* vehicle_ros, const std::string& camera_name, const CameraSetting& camera_setting)
{
    ...
    if (isENU_) {
        std::swap(static_cam_tf_body_msg.transform.translation.x, static_cam_tf_body_msg.transform.translation.y);// @suppress("Invalid arguments")
        std::swap(static_cam_tf_body_msg.transform.rotation.x, static_cam_tf_body_msg.transform.rotation.y);// @suppress("Invalid arguments")
...
void AirsimROSWrapper::process_and_publish_img_response(const std::vector<ImageResponse>& img_response_vec, const int img_response_idx, const std::string& vehicle_name)
{
    ...
    for (const auto& curr_img_response : img_response_vec) {
        ...
        if (curr_img_response.pixels_as_float) {
            image_pub_vec_[img_response_idx_internal].publish(get_depth_img_msg_from_response(curr_img_response,// @suppress("Invalid arguments")
                                                                                              curr_ros_time,
                                                                                              curr_img_response.camera_name + "_optical"));
        }
        // Scene / Segmentation / SurfaceNormals / Infrared
        else {
            image_pub_vec_[img_response_idx_internal].publish(get_img_msg_from_response(curr_img_response, // @suppress("Invalid arguments")
                                                                                        curr_ros_time,
                                                                                        curr_img_response.camera_name + "_optical"));
        }
...
void AirsimROSWrapper::publish_camera_tf(const ImageResponse& img_response, const ros::Time& ros_time, const std::string& frame_id, const std::string& child_frame_id)
{
    ...
    if (isENU_) {
        std::swap(cam_tf_body_msg.transform.translation.x, cam_tf_body_msg.transform.translation.y);// @suppress("Invalid arguments")
        std::swap(cam_tf_body_msg.transform.rotation.x, cam_tf_body_msg.transform.rotation.y);// @suppress("Invalid arguments")
